  ...
  server.route({
    ...
    handler: async (request, h) => {
      try {
        ...
        debug(`Scope: ${scope}, Url2: ${outboundUrl}`);
  ...
  server.route({
    ...
    handler: async (request, h) => {
      try {
        ...
        debug(`Payload: ${JSON.stringify(payload, null, 4)}`);
        // debug(`Method: ${method}, Url2: ${config.redirectUri}?${qs.stringify(payload)}`);
        ...
        const { policyName, nonce, code_verifier } = savedState
        ...
        const [authorizationErr, tokenSet] = await to(internals.root.retryable(() => client.callback(authRedirectUriFqdn, requestParams, {
          state,
          code_verifier,
          nonce
        }), internals.root.retryable.b2cRequestRetry))
    ...
    server.route({
      ...
      handler: async (request, h) => {
        try {
          ...
          debug(`Method: ${request.method}, Url2: ${request.url}`);
          ...
          debug(`Method: ${request.method}, Url2: ${redirectUrl}`);
